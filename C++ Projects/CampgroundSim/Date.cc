...
// Returns true if date in question is before argument date, otherwise false (equal is false)
...
// Returns true if date in question is after argument date, otherwise false, (equals is false)
bool Date::moreThan(Date &d) {
	if (year == d.year) {
		if (month == d.month) {
			return (day > d.day);
		}
